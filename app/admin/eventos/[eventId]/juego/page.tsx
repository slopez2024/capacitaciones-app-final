...
export default function JuegoPage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  // Detectar si todas las preguntas terminaron → mostrar podio
  useEffect(() => {
  ...
  const buildRanking = async () => {
    ...
    for (const answer of allAnswers) {
      ...
      if (q.type === 'true_false') {
        const correctOpt = q.question_options.find((o: { is_correct: boolean }) => o.is_correct)
        correct = answer.answer_text === (correctOpt?.text === 'Verdadero' ? 'true' : 'false')
      } else {
        const correctOptions = q.question_options.filter((o: { is_correct: boolean }) => o.is_correct).map((o: { id: string }) => o.id)
  ...
  const isCorrect = () => {
    ...
    if (question.type === 'true_false') {
      ...
      return myAnswer.answerText === (correctOpt?.text === 'Verdadero' ? 'true' : 'false')
    } else {
  ...
  if (allClosed) {
    return (
      <div className="min-h-screen bg-[#0f0f1a] text-white flex flex-col">
        <header className="bg-indigo-700 px-4 py-3 text-center">
          <p className="text-sm font-medium">🎓 CapacitApp</p>
        </header>
        <main className="flex-1 flex flex-col items-center justify-center p-4">
          <h2 className="text-3xl font-bold mb-1 text-center">🏆 Podio Final</h2>
          <p className="text-white/50 text-sm mb-8">Resultados de la capacitación</p>
              ...
              {ranking.map((item, i) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-white">{item.apellido}, {item.nombre}</p>
                    <p className="text-white/50 text-xs">{item.correct} correctas</p>
                  </div>
                  <div className="text-right">
                    <p className="text-xl font-bold text-indigo-300">{item.points}</p>
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        {loading ? (
          ...
          <div className="text-center px-4">
            ...
            {timeLeft !== null && (
              <p className="text-3xl font-bold text-indigo-600 mt-4">{timeLeft}s</p>
            )}
          </div>
        ) : (
