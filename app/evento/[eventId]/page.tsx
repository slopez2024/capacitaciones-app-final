...
export default function EventoPage({params}:{params:Promise<{eventId:string}>}){
  ...
  const handleSubmit=async(e:React.FormEvent)=>{
    ...
    setError("")
    setLoading(true)
    const supabase=createClient();const timeout=setTimeout(()=>{setError("Error de conexion. Intentá de nuevo.");setLoading(false)},10000)
    ...
    if(!event){setError("Evento no encontrado.");clearTimeout(timeout);setLoading(false);return}
    ...
    clearTimeout(timeout);window.location.href="/evento/"+eventId+"/juego"
  }
