...
export default function JuegoPage({params}:{params:Promise<{eventId:string}>}){
  ...
  const handleAnswer=async(optionId?:string,answerText?:string)=>{
    ...
    const s=createClient()
    const {error:err}=await s.from("answers").insert({question_id:question.id,attendee_id:attendeeId,event_id:eventId,option_id:optionId||null,answer_text:answerText||null,response_time_ms:responseTimeMs})
        ...
        :questionIsClosed||timeLeft===0?(
          ...
          <div className="w-full max-w-sm">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
              {displayQuestion.image_url&&<img src={displayQuestion.image_url} alt="" className="w-full rounded-xl mb-4 max-h-48 object-cover"/>}
              <p className="text-slate-800 font-semibold text-base mb-5 leading-snug">{displayQuestion.text}</p>
              <div className="space-y-3">
                {displayQuestion.type==="true_false"?(
                  <>
                ...
                ):displayQuestion.question_options?.sort((a,b)=>a.order_num-b.order_num).map(opt=>(
