        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              ...
              <input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" placeholder="••••••••" required/>
            </div>
            <ErrorMessage message={error}/>
            <button type="submit" disabled={loading} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2.5 rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
              {loading?<Spinner size="sm"/>:"Ingresar"}
            </button>
          </form>
          <div className="mt-4 pt-4 border-t border-slate-100">
            <button onClick={handleRegister} disabled={loading} className="w-full text-sm text-slate-500 hover:text-indigo-600 transition-colors">
